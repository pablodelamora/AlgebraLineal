...
void resuelveProblema(int[9][1], int[9][1], int [9][1], int);
void multiplicaMatrices(int[9][1], int[9][1]);
...
int main(){
	...
	while (m != 3){
		...
		case 1: cout << "\t\t\tCrear la matriz inicial\n\n";
				...
				resuelveProblema(sumaMatInicialFinal, matInicial, matFinal, mod);
				break;
...
void resuelveProblema(int sumaMatInicialFinal[9][1], int matInicial[9][1], int matFinal[9][1], int mod){
	for (int i = 0; i < 9; i++){
		sumaMatInicialFinal[i][0] = (matInicial[i][0] + matFinal[i][0])%mod;
		cout << matInicial[i][0] << endl << matFinal[i][0] << endl;
	}
	multiplicaMatrices(sumaMatInicialFinal, matFinal);


}

void multiplicaMatrices(int sumaMatInicialFinal[9][1], int matResultados[9][1]){
	int Ainv[9][9];
	Ainv[0][0] = 1;
	Ainv[1][0] = 1;
	Ainv[2][0] = 1;
	...
	for (int i = 0; i < 9; i++){
		for (int j = 0; j < 9; j++){
			matResultados[i][1] = matResultados[i][1] + (Ainv[i][j] * sumaMatInicialFinal[j][0]);
		}
	}
